...
contract TokenLocker is Ownable, ReentrancyGuard {
    ...
    // The address where the fees will be sent
    ...
    constructor(address _initialOwner, address _feeRecipient) payable Ownable(_initialOwner) {
    ...
    function lock(
        ...
        id = _createLock(_owner, token, isLpToken, amount, unlockDate, 0, 0, 0, description);
        _safeTransferFromEnsureExactAmount(token, msg.sender, address(this), amount);
        emit LockAdded(id, token, _owner, amount, unlockDate);
        return id;
    }
    ...
    function multipleVestingLock(
        ...
        require(cycleBps > 0 && cycleBps < 10_000, "Invalid bips for cycle");
        require(tgeBps + cycleBps <= 10_000, "Sum of TGE bps and cycle should be less than 10000");
        return _multipleVestingLock(owners, amounts, token, isLpToken, [tgeDate, tgeBps, cycle, cycleBps], description);
    }
    ...
    function _multipleVestingLock(
        ...
        _safeTransferFromEnsureExactAmount(token, msg.sender, address(this), sumAmount);
        return ids;
    }
    ...
    function _normalUnlock(Lock storage userLock) internal {
        ...
        uint256 fee = unlockAmount / 100; // Calculate the 1% fee
        uint256 amountAfterFee = unlockAmount - fee; // Subtract the fee from the unlock amount

        userLock.unlockedAmount = amountAfterFee;

        _tokenToLockIds[userLock.token].remove(userLock.id);

        IERC20(userLock.token).safeTransfer(msg.sender, amountAfterFee); // Transfer the tokens to the owner
        IERC20(userLock.token).safeTransfer(feeRecipient, fee); // Transfer the fee to the specified address

        emit LockRemoved(userLock.id, userLock.token, msg.sender, amountAfterFee, block.timestamp);
    }
    ...
    function _vestingUnlock(Lock storage userLock) internal {
        ...
        uint256 fee = withdrawable / 100; // Calculate the 1% fee
        uint256 amountAfterFee = withdrawable - fee; // Subtract the fee from the withdrawable amount

        userLock.unlockedAmount = newTotalUnlockAmount - fee;

        IERC20(userLock.token).safeTransfer(userLock.owner, amountAfterFee); // Transfer the tokens to the owner
        IERC20(userLock.token).safeTransfer(feeRecipient, fee); // Transfer the fee to the specified address
    ...
    function _withdrawableTokens(Lock memory userLock) internal view returns (uint256) {
        ...
        uint256 tgeReleaseAmount = Math.mulDiv(userLock.amount, userLock.tgeBps, 10_000);
        uint256 cycleReleaseAmount = Math.mulDiv(userLock.amount, userLock.cycleBps, 10_000);
    ...
    function WithdrawETH(address payable _reciever, uint256 _amount) public onlyOwner {
