...
contract AdvancedToken is ERC20, Ownable, IERC721Receiver {
    ...
    function updateTaxFee(uint256 _buyTaxFee, uint256 _sellTaxFee) external onlyOwner {
        require(_sellTaxFee <= 200000, "sell fee should be less than 20%");
        require(_buyTaxFee <= 200000, "buy fee should be less than 20%");

        buyFee = _buyTaxFee;
        sellFee = _sellTaxFee;
    }

    function updateRewardFee(uint256 _buyRewardFee, uint256 _sellRewardFee) external onlyOwner {
        require(_sellRewardFee <= 200000, "sell fee should be less than 20%");
        require(_buyRewardFee <= 200000, "buy fee should be less than 20%");

        buyReward = _buyRewardFee;
        sellReward = _sellRewardFee;
    }

    function updateLpFee(uint256 _lpBuyFee, uint256 _lpSellFee) external onlyOwner {
        require(_lpSellFee <= 200000, "sell fee should be less than 20%");
        require(_lpBuyFee <= 200000, "buy fee should be less than 20%");

        lpBuyFee = _lpBuyFee;
        lpSellFee = _lpSellFee;
    }

    function updateBurnFee(uint256 _buyBurnFee, uint256 _sellBurnFee) external onlyOwner {
        require(_sellBurnFee <= 200000, "sell fee should be less than 20%");
        require(_buyBurnFee <= 200000, "buy fee should be less than 20%");
    ...
    function updateMaxTransaction(uint256 _maxTransaction) external onlyOwner {
    ...
    function setWhiteList(address _addr, bool value) public onlyOwner {
        require(whiteList[_addr] != value, "This address is already set to that value");
        ...
        if (whiteList[from] == true || whiteList[to] == true) {
            ...
            if (automatedMarketMakerPairs[from]) {
                ...
                if (buyReward != 0) {
                    ...
                    } else if (rewardToken == 2) {
                        //reward token type is eth, for this sell rewardFee and send it to to wallet
            ...
            // Sell
                ...
                if (sellReward != 0) {
                    ...
                    if (rewardToken == 1) {
                        // reward token type is base token
                        amount = amount - _rewardFee;
                    } else if (rewardToken == 2) {
                        // reward token type is eth
                        require(dexType == 2, "Can't work reward for V3!");
                        super._update(from, address(this), _rewardFee);
                        initialBalance = address(this).balance;
                        swapTokensToETH(_rewardFee);
